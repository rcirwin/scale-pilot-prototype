...
// ---------------------------------------------------------------------------
// Production SI nav items — rendered as inline SVGs with per-item colors
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Scale Pilot nav items — Lucide icons
// ---------------------------------------------------------------------------
...
const pilotItems: PilotNavItem[] = [
  { label: 'Scale Pilot', color: '#45a19c', icon: Sparkles, href: '/' },
  { label: 'Scale Optimizer', color: '#45a19c', icon: Zap, href: '/optimizer' },
  { label: 'Activity', color: '#45a19c', icon: Activity, href: '/activity' },
  { label: 'Chat', color: '#45a19c', icon: MessageSquare, href: '/chat' },
  { label: 'Settings', color: '#6c757d', icon: Settings, href: '/settings' },
];

// ---------------------------------------------------------------------------
// SI Logo SVG — white circle with dark bar chart + trend arrow
// ---------------------------------------------------------------------------
...
function SILogo() {
  return (
    <svg viewBox="0 0 40 40" width="32" height="32">
      ...
      <path d="M12 28 L12 20 L16 20 L16 28 Z" fill="#151c29" />
      <path d="M18 28 L18 16 L22 16 L22 28 Z" fill="#151c29" />
      <path d="M24 28 L24 12 L28 12 L28 28 Z" fill="#151c29" />
      <path d="M10 14 L18 10 L28 8" stroke="#151c29" strokeWidth="2" fill="none" strokeLinecap="round" />
      <path d="M25 6 L29 8 L26 11" stroke="#151c29" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function NavTooltip({ label, visible }: { label: string; visible: boolean }) {
  ...
  return (
    <div className="absolute left-[56px] top-1/2 -translate-y-1/2 z-[100] pointer-events-none" style={{ whiteSpace: 'nowrap' }}>
      <div className="bg-[#1a2332] text-white text-xs font-medium px-2.5 py-1.5 rounded shadow-lg border border-white/10">
        {label}
      </div>
    </div>
  );
...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen flex flex-col items-center z-50" style={{ width: 56, backgroundColor: '#151c29' }}>
      {/* Logo */}
      ...
      {/* Nav */}
      <nav className="flex-1 flex flex-col w-full overflow-y-auto">
        {/* Production SI items (non-functional, no active state) */}
        {productionItems.map((item) => (
          ...
          >
            <div
              className="flex items-center justify-center w-full cursor-default transition-colors hover:bg-white/5"
              style={{ height: 48, color: item.color }}
            >
              <svg viewBox={item.viewBox} width="20" height="20" fill="currentColor">
                <path d={item.path} />
              </svg>
            </div>
            <NavTooltip label={item.label} visible={hoveredItem === item.label} />
          </div>
        ))}

        {/* Divider */}
        <div className="flex items-center justify-center py-2">
          <div style={{ width: 32, height: 1, backgroundColor: '#253147' }} />
        </div>

        {/* Scale Pilot items (functional, with active state) */}
        {pilotItems.map((item) => {
